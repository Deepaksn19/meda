...
| 6 | Initial actuations `N` | sampled per episode (Alg. 2); healthy training resets them | zeros during training; `U{0, 399}` in the bioassay scheduler | zeros for training; `U{0, 399}` for bioassays (both configurable). Exception: `covid_60x30.yaml` trains on `U{0, 399}` pre-aged chips, matching the chips it routes on in the bioassays (Alg. 2 samples `N`; the authors' 60×30 agent was trained on healthy chips). |
...
| 8 | `k_max` | `α(Wh + Hh)` with `α ∈ [1, 2]`; the text also says `2(W + H)` | `W + H` of the chip during training; `1·(Wh + Hh)` for bioassays | `α = 1` on the hazard zone (`kmax_alpha`); `kmax_basis: chip` switches to `W + H` of the chip, as in the reference training runs. The budget matters when comparing *untrained* agents with Figs. 4/7. Timed-out episodes count `k_max` cycles, so with `α = 1` the mean cycles cannot exceed E[`Wh + Hh`] ≈ 39/65/117/163 for 30/60/120/180 chips. The early epochs of Fig. 4 (random init) and Fig. 7 sit higher (≈ 44/83/150/≥ 230). `kmax_alpha: 2`, still inside the paper's range, or `kmax_basis: chip` matches that scale; converged agents are unaffected. An episode ends after `k_max` cycles, as in Alg. 2 (`k ≥ k_max`); the code's `step_count > max_step` allows one more. Timeouts are *truncations*, not terminal failures, so PPO bootstraps them. This avoids the state aliasing the paper warns about. `timeout_terminal: true` reproduces PPO2's cut-off instead. |
...
| 16 | PPO settings | PPO2, 8 environments | saved model: `n_steps=64`, `nminibatches=16`, `noptepochs=4`, `γ=0.99`, `λ=0.95`, `ent=0.01`, `vf=0.5`, `max_grad_norm=0.5`, `clip=0.2`, `clip_vf=0.2` | Same settings, translated to SB3: `batch_size = 8·64/16 = 32`, `clip_range_vf = 0.2`, and **`vf_coef = 0.25`**, because PPO2's value loss is `0.5·mean(max(unclipped, clipped))` while SB3's is a plain mean. SB3 also uses only the clipped value prediction instead of the maximum. As in Alg. 2 (`resample ← ⊤`) and `PPO2.learn()`, the environments are reset at the start of every epoch. |
| 17 | LR schedule | per-epoch rule, `η0 = 3.5e-4`, decay 0.7 if success > 0.99, floor 1e-6 | same rule, but decays only at exactly 100% success. Within each epoch the rate is `η_i·sqrt(remaining fraction)`. | **Paper** threshold, plus the code's within-epoch sqrt decay (`intra_epoch: sqrt`, or `constant`). The logged 0825a run actually *halved* the base rate after each 100% epoch, with no floor (3.5e-4 → 1.7e-4 → … → 3e-25). `reference_0825a_30x30.yaml` mirrors that. |
| 18 | Epoch length | 2^14 steps | 16384 in the 0825a run | 2^14 |
| 19 | Per-epoch evaluation | 500 random jobs | 500 deterministic episodes on a separate environment | Same, but the eval environment is **re-seeded every epoch** (ours). Jobs and chips are drawn from a random stream separate from the droplet dynamics, so every epoch sees exactly the same 500 jobs and the curves are less noisy. |
...
| 21 | Transfer learning | initialize from the pretrained agent | loads the whole PPO2 model | Weights only, with a fresh optimizer and the new stage's hyperparameters (ours). Fault stages start from the healthy agent of the same size, as Sec. V-B describes. Fig. 3(b) instead draws the 10% chain across sizes; see the comment in `transfer_learning.yaml`. |
...
| 24 | Bioassays | COVID-RAT, COVID-PCR; no job lists | `meda_sgs.py`: `sg_CRAT` (28 operations), `sg_CPCR` (34 operations), 60×30 chip, 4×4 droplets, 1000 trials, `k_max = 2000` | Transcribed from the code (`bioassay/library.py`). A test checks the transcription against the reference file, downloaded from a pinned commit (set `MEDA_REFERENCE_SGS` to a local copy to run it offline). Deviations:<br>• The second split droplet goes to `goals[1]`; the code sends it to `goals[0]`, a copy-paste bug.<br>• Timed-out jobs keep routing (`on_timeout: continue`); the code treats them as done (`skip`).<br>• All jobs sense and wear one shared chip; the code gives each job a snapshot copy.<br>• `cycles` counts executed control cycles and failed trials are `inf`; the code adds one detection tick and records failures as 2000 cycles.<br>Like the code, each operation starts from its *listed* coordinates, which are not always where the previous operation left the droplet (e.g. COVID-PCR `n09`, COVID-RAT `n09`/`n10`). |
